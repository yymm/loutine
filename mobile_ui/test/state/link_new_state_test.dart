...
void main() {
  group('LinkNewState', () {
    test('初期値はurl空文字列、title空文字列', () {
      ...
      final state = container.read(linkNewStateProvider);
      expect(state.url, '');
      expect(state.title, '');
    });
    ...
    test('changeUrl()でURLが変更される', () {
      ...
      container.read(linkNewStateProvider.notifier).changeUrl('https://example.com');
      final state = container.read(linkNewStateProvider);
    ...
    test('changeTitle()でタイトルが変更される', () {
      ...
      container.read(linkNewStateProvider.notifier).changeTitle('テストタイトル');
      final state = container.read(linkNewStateProvider);
      expect(state.url, '');
    ...
    test('changeUrlとchangeTitle両方で値が変更される', () {
      ...
      final notifier = container.read(linkNewStateProvider.notifier);
